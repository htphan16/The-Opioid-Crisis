...
def info(df):
    ...
    for counties in df['GEO.display-label']:
        ...
        county = counties[0]
        if county != 'Geography':
            counties_list.append(county.upper())
        try:
            state = counties[2]
            if state == 'Kentucky':
...
# Total Households in each county in each state
def thh_yy_c(df, yy):
    return df.rename(columns={'Total Households': 'Total Households in {}'.format(yy)})
    for county in df['COUNTY']:
        df['COUNTY'] = county.upper()

thh_c = thh_yy_c(d_10, '2010').merge(thh_yy_c(d_11, '2011'), left_index=True, right_index=True)

print(thh_c)

ky_drugs = pd.read_csv('kentucky_counties_sort_by_2010.csv')
oh_drugs = pd.read_csv('ohio_counties_sort_by_2010.csv')
pa_drugs = pd.read_csv('pennsylvania_counties_sort_by_2010.csv')
va_drugs = pd.read_csv('virginia_counties_sort_by_2010.csv')
wv_drugs = pd.read_csv('west-virginia_counties_sort_by_2010.csv')
